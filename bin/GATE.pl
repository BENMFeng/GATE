...
$ perl bin/GATE.pl qc aln var exp --config=config.txt --prefix=test
...
my @cmd_ary = qw(bc qc aln var exp diff as peak plot denovo predict fusion phylogen);
...
foreach my $cmd(@cmd_ary){
	if (exists $gate->{'cmd'}{$cmd}){
		...
		if ($cmd eq "as" && !exists $gate->{'cmd'}{"aln"}) {
			...
			open (OUT,">$worksh") || die $!;
			if (defined $Auto) {
				print OUT $gate->{"CustomSetting:PBS-header"};
			}
			print OUT $gate->aln;
			...
			$allwork.=qq(echo \`date\`\necho "$worksh done"\n);
			#if (defined $Auto) {
			#	my $qsub_para=$gate->{"CustomSetting:qsub"};
